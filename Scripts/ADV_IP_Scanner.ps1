...
function Update-OUIList {
    ...
    Out-File -InputObject $Output -FilePath $outputPath
}
...
# Check if the file exists
	...
	$button_ping_Click = {
		Add-Logs -text "Network Scanner"
		$button_ping.Enabled = $False
		if (-Not (Test-Path "C:\Revention\Utilities\oui.txt")) {
			Update-OUIList
		}
		$result = Invoke-IPv4NetworkScan -StartIPv4Address 192.168.192.1 -EndIPv4Address 192.168.192.254 -EnableMACResolving | Format-Table -AutoSize
